...
export function acceptSuggestion(editor: Editor, suggestionId: string): boolean {
  ...
  SlateEditor.withoutNormalizing(editor, () => {
    for (const path of insertionPaths) {
      Transforms.unsetNodes(editor, ['suggestionId', 'suggestionInsertion', 'suggestionDeletion', 'suggestionDeletionKind', 'reviewInsert', 'reviewDelete', 'authorId', 'authorColor'], {
        at: path,
        match: (n) => Text.isText(n),
        split: false,
      })
